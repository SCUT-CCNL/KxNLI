...
class snli_Data(data.DataLoader):
    def __init__(self,source_data_name="train.csv",target_data_name="train.csv",
        ...
        self.source_data, self.target_data_input, self.target_data, self.rationales_data, \
        self.graph_embs, self.labels, self.prem_with_r, self.hypo_with_r = self.load_data(with_rationale=True)

    def load_data(self, with_rationale=True):
        global cpts2emb, concept2id

        raw_source_data, raw_target_data, raw_rationales, \
        prem_with_r, hypo_with_r, graph_embs, labels = [], [], [], [], [], [], []
        with open("./data/esnli/raw_data/"+self.source_data_name, 'r') as csv_file, \
            ...
            for row_count, row in enumerate(csv_reader):
                if row_count == 0:
                    continue
                cpts = json.loads(cpt_lines[row_count - 1])
                if cpts != []:
                    ...
                    if with_rationale:
                        if 'train' in self.source_data_name:
                            ...
                            prem_with_r.append(row[6])
                            hypo_with_r.append(row[7])
                        else:
                            raw_rationales.append(( ' '.join(re.findall(r'\*(.*?)\*', row[5]+' '+row[6]))))
                            prem_with_r.append(row[5])
                            hypo_with_r.append(row[6])
                else: continue
        ...
        return source_data,target_data_input,target_data,rationales_data,\
               graph_embs, labels, prem_with_r, hypo_with_r
    ...
    def __getitem__(self, idx):
        return self.source_data[idx], self.target_data_input[idx], self.target_data[idx], self.rationales_data[idx], \
               self.graph_embs[idx], self.labels[idx], self.prem_with_r[idx], self.hypo_with_r[idx]
