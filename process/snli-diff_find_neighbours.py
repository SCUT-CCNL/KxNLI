...
def find_neighbours_frequency(source_sentence, source_concepts, target_concepts, T):
    ...
    for t in range(T):
        ...
        V = list(V.items())
        count_V = sorted(V, key=lambda x: x[1], reverse=True)
...
def process(input_path, output_path, T):
    data = []
    ...
    for ex in tqdm(data):
        ...
        e, found, avg_nodes = find_neighbours_frequency(ex['sent'], source, target, T)
        avg_len += avg_nodes
        with open(output_path, 'a+') as f:
            json.dump(e, f)
            f.write('\n')
    print('{} hops avg nodes: {}'.format(T, avg_len / i) )
...
types = ['train.json', 'dev.json', 'test.json']
for type in types:
    process("./diff/mycpts_"+type, ('./diff/hop_my'+type), T)