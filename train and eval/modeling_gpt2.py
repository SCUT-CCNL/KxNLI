...
class MultiHopGen(GPT2PreTrainedModel):
    def __init__(self, config=config, source_length=0, gamma=0.8, alpha=1, beta=1, aggregate_method="max", tokenizer=None, hop_number=2):
        super(MultiHopGen, self).__init__(config)
        self.transformer = GPT2Model(config, source_length)

        self.lm_head = nn.Linear(config.n_embd, config.vocab_size, bias=False)

        self.linear_concept = nn.Linear(config.n_embd, 399, bias=False)
    ...
    def comp_gcn(self, concept_hidden, relation_hidden, head, tail, concept_label, triple_label, layer_idx):
        ...
        act = nn.ReLU()
        e_ur = torch.bmm(update_node, concept_hidden.transpose(1,2))
        alpha_ur = torch.softmax(e_ur, dim=0)
        update_node = torch.bmm(alpha_ur, update_node)
        update_node = self.W_s[layer_idx](concept_hidden) + update_node / count_out.clamp(min=1).unsqueeze(2)
    ...
    def multi_hop2(self, concept_ids, relation, head, tail, concept_label, triple_label):
        rel_repr = self.relation_embd(relation)
        concept_hidden = self.transformer.wte(concept_ids)

        node_repr, rel_repr = self.multi_layer_comp_gcn(concept_hidden, rel_repr, head, tail, concept_label, triple_label)
        concept_prob = self.linear_concept(node_repr)
        return concept_prob
    ...
    def forward(self, src_input_ids, attention_mask, src_position_ids, 
        ...
        hybrid_probs, gate, triple_score = self.autoreg_forward(node_repr, input_ids, position_ids,
                            attention_mask,
        ...
        loss = gen_loss + self.alpha * gate_loss

        
        return loss, gen_loss, gate_loss
    ...
    def generate(self, src_input_ids, attention_mask, src_position_ids, 
        ...
        return seq_generator.generate(self.autoreg_forward, sample, memory, node_repr)

    def autoreg_forward(self, node_repr, input_ids, position_ids, attention_mask, memory_dict, do_generate=False, lm_mask=None):
        ...
        if do_generate:
            ...
            node_repr = node_repr.repeat(3, 1, 1)
        ...
        triple_logits = torch.matmul(hidden_states, node_repr.transpose(1,2))
        triple_score = sigmoid(triple_logits)
        unorm_cpt_probs = sigmoid(triple_logits)
